                                 ...
                                 Sim_time check_time, Sim_time duration)
{
    while (true)
    {
        Sim_time next_available = is_time_available(use_effect_timers, check_time, duration);
...
std::vector<std::pair<Sim_time, Use_effect>> compute_use_effect_order(std::vector<Use_effect>& use_effects,
                                                                      ...
                                                                      Sim_time sim_time, double total_ap,
                                                                      int number_of_targets, double init_rage)
{
    ...
    else
    {
        ...
        {
            Sim_time test_time{Sim_time::zero()};
            for (int i = 0; i < 10; i++)
            {
...
void shuffle_bloodrage(std::vector<std::pair<Sim_time, Use_effect>>& use_effect_timers, double init_rage)
{
    ...
    {
        if (reverse_it->first.milliseconds() < 0)
        {
            projected_rage += reverse_it->second.rage_boost;
            if (projected_rage < 0.0)
            {
                // Reshuffle bloodrage since there is no rage
                auto bloodrage_reverse_it = std::find_if(
                    use_effect_timers.rbegin(), use_effect_timers.rend(),
                    [](const std::pair<Sim_time, Use_effect>& ue) { return ue.second.name == "Bloodrage"; });
                if (bloodrage_reverse_it != use_effect_timers.rend())
                {
                    ...
                    bloodrage_forward_it->first = reverse_it->first - Sim_time::from_seconds(1);
...
void sort_use_effect_order(std::vector<std::pair<Sim_time, Use_effect>>& use_effect_order, Sim_time sim_time)
{
    ...
    std::sort(use_effect_order.begin(), use_effect_order.end(),
              [](const std::pair<Sim_time, Use_effect>& a, const std::pair<Sim_time, Use_effect>& b) {
                                    ...
                                    Sim_time sim_time_ms)
{
    ...
    return ap_during_active * std::min(use_effect.duration / sim_time_ms, 1);
}
